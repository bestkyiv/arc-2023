...
function FlippingCard({category}) {
 ...
 const isCaseStudy = category.header === "Case study";
  ...
  return (
    <div className={`${classes["flip-card"]} ${isFlipped ? classes['flipped'] : ''}`} onClick={handleCardFlip}>
      <div className={classes['flip-card-inner']}>
        <div className={classes["flip-card-front"]}>
          <img src={`/${isCaseStudy ? "case-study" : "innovative-design"}-front.svg`} alt={isCaseStudy ? "Case Study": "Innovative design"} className={classes.img}/>
        </div>
